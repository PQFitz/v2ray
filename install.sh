...
author=233boy
# github=https://gh.cdn.fullcone.cn/https://github.com/PQFitz/v2ray
...
download() {
    case $1 in
    core)
        link=https://gh.cdn.fullcone.cn/https://github.com/${is_core_repo}/releases/latest/download/${is_core}-linux-${is_core_arch}.zip
        [[ $is_core_ver ]] && link="https://gh.cdn.fullcone.cn/https://github.com/${is_core_repo}/releases/download/${is_core_ver}/${is_core}-linux-${is_core_arch}.zip"
        name=$is_core_name
        tmpfile=$tmpcore
        is_ok=$is_core_ok
        ;;
    sh)
        link=https://gh.cdn.fullcone.cn/https://github.com/${is_sh_repo}/releases/latest/download/code.zip
        name="$is_core_name 脚本"
        tmpfile=$tmpsh
        is_ok=$is_sh_ok
        ;;
    jq)
        link=https://gh.cdn.fullcone.cn/https://github.com/jqlang/jq/releases/download/jq-1.7rc1/jq-linux-$is_jq_arch
        name="jq"
        tmpfile=$tmpjq
        is_ok=$is_jq_ok
        ;;
...
# get server ip
get_ip() {
    export "$(_wget -6 -qO- https://cloudflare.com/cdn-cgi/trace | grep ip=)" &>/dev/null
}
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        case $1 in
        online)
            err "如果想要安装旧版本, 请转到: https://gh.cdn.fullcone.cn/https://github.com/233boy/v2ray/tree/old"
            ;;
        -f | --core-file)
            [[ -z $2 ]] && {
...
exit_and_del_tmpdir() {
    ...
    [[ ! $1 ]] && {
        msg err "哦豁.."
        msg err "安装过程出现错误..."
        echo -e "反馈问题) https://gh.cdn.fullcone.cn/https://github.com/${is_sh_repo}/issues"
        echo
        exit 1
    }
    exit
